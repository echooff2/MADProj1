...
\newcommand{\getAccuracyNn}{72.93}
\newcommand{\getAccuracyMixednn}{72,22}
\newcommand{\getaccuracyMixedsr}{72.48}

%% AUC
\newcommand{\getAucKnn}{0,795}
\newcommand{\getAucTree}{0,782}
\newcommand{\getAucNn}{0,805}
\newcommand{\getAucMixedNn}{0,802}
\newcommand{\getAucMixedSr}{0,804}
...
metod klasyfikacji: sieci neuronowej, drzewa klasyfikacyjnego oraz KNN,
...
\end{table}
\indent Na podstawie powyższego możemy usunąć następujące cechy: 

\textit{blueFirstBlood, redDeaths, blueDeaths, blueTotalGold, blueAvgLevel, blueTotalMinionsKilled, blueGoldDiff, blueExperienceDiff, redTotalGold, redAvgLevel, redTotalMinionsKilled}
...
W niniejszym rozdziale przedstawiono metody wykorzystane do budowy modeli predykcyjnych oraz oceny ich skuteczności. Analizie poddano klasyczne algorytmy uczenia maszynowego, takie jak drzewo klasyfikacyjne i metoda k najbliższych sąsiadów, a także sieć neuronową oraz modele hybrydowe łączące predykcje kilku klasyfikatorów. Dla każdej metody omówiono podstawy teoretyczne, sposób konstrukcji modelu oraz procedurę uczenia i walidacji. Na końcu przedstawiono miary wykorzystane do oceny jakości otrzymanych klasyfikatorów.
...
Sieci neuronowe \cite{NN} są modelami uczenia maszynowego inspirowanymi działaniem biologicznych sieci neuronowych. Składają się z połączonych ze sobą neuronów tworzących kolejne warstwy przetwarzające dane wejściowe. Dzięki zastosowaniu nieliniowych funkcji aktywacji sieci neuronowe są zdolne do modelowania złożonych zależności pomiędzy zmiennymi.
	...
	\begin{equation}
		\hat{y} = \sigma(f(\mathbf{x}, \Theta))
	\end{equation}
	
	gdzie:
	\begin{itemize}
		\item $\mathbf{x}$ -- wektor cech wejściowych,
		\item $\Theta$ -- zbiór wszystkich parametrów modelu (wag i obciążeń),
		\item $f(\cdot)$ -- transformacja nieliniowa realizowana przez warstwy ukryte,
	...
	\end{equation}
	i zwraca wartość z przedziału $(0,1)$, interpretowaną jako prawdopodobieństwo przynależności do klasy pozytywnej ($p$).
	
	Zgodnie ze specyfikacją danych, wektor wejściowy $\mathbf{x}$ składa się z $n=16$ cech należących do różnych dziedzin. Przestrzeń wejściową modelu można sformalizować jako:
	\begin{equation}
		\mathbf{x} \in \mathbb{Z}^2 \times \{0,1\}^2 \times \mathbb{Z} \times \mathbb{R} \times \mathbb{Z}^2 \times \{0,1\}^3 \times \mathbb{Z} \times \mathbb{R} \times \mathbb{Z}^2 \times \mathbb{R}
	\end{equation}
	co w zapisie wektorowym przyjmuje postać:
	\begin{equation}
		\mathbf{x} = [x_1, x_2, \dots, x_{16}]^T
	\end{equation}
	
	Zastosowana architektura sieci składa się z następujących warstw:
	\begin{itemize}
		\item \textbf{Warstwa wejściowa:} przyjmująca wektor \textbf{x}.
		\item \textbf{Pierwsza warstwa ukryta:} zawierająca 128 neuronów z funkcją aktywacji ELU (ang. \textit{Exponential Linear Unit}):
		\begin{equation}
			f_1(z) =
			\begin{cases}
				e^z - 1 & \text{dla } z \le 0 \\
				z & \text{dla } z > 0
			\end{cases}
		\end{equation}
		\item \textbf{Druga warstwa ukryta:} zawierająca 64 neurony z funkcją aktywacji Softplus:
		\begin{equation}
			f_2(z) = \ln(1 + e^z)
		\end{equation}
		\item \textbf{Warstwa wyjściowa:} zawierająca jeden neuron z funkcją sigmoidalną $\sigma(z)$.
	\end{itemize}
	
	W celu ograniczenia zjawiska przeuczenia (ang. \textit{overfitting}), pomiędzy warstwami ukrytymi zastosowano mechanizm \textit{dropout} z prawdopodobieństwem zachowania neuronu $p_{drop}=0.5$. Polega on na losowym zerowaniu wyjść części neuronów podczas procesu uczenia.
	
	Podsumowując, pełne przejście w przód (ang. \textit{forward pass}) w fazie predykcji (przy wyłączonym mechanizmie \textit{dropout}) można zapisać jako złożenie funkcji. Ostateczne równanie na prawdopodobieństwo wygranej ($p$) przyjmuje postać:
	
	\begin{equation}
		p = P(Y=1 \mid \mathbf{x}) = \sigma \Big( \mathbf{W}^{(3)} f_2 \big( \mathbf{W}^{(2)} f_1 (\mathbf{W}^{(1)} \mathbf{x} + \mathbf{b}^{(1)}) + \mathbf{b}^{(2)} \big) + \mathbf{b}^{(3)} \Big)
	\end{equation}

\newpage
	gdzie poszczególne parametry modelu $\Theta$ to macierze wag $\mathbf{W}^{(k)}$ oraz wektory obciążeń (ang. \textit{bias}) $\mathbf{b}^{(k)}$ dla kolejnych warstw:
	\begin{itemize}
		\item $\mathbf{W}^{(1)} \in \mathbb{R}^{128 \times 16}, \quad \mathbf{b}^{(1)} \in \mathbb{R}^{128}$
		\item $\mathbf{W}^{(2)} \in \mathbb{R}^{64 \times 128}, \quad \mathbf{b}^{(2)} \in \mathbb{R}^{64}$
		\item $\mathbf{W}^{(3)} \in \mathbb{R}^{1 \times 64}, \quad \mathbf{b}^{(3)} \in \mathbb{R}^1$
	\end{itemize}
...
Model sieci neuronowych działa  przy użyciu standardowej dla naszej pracy pięciokrotnej walidacji Monte Carlo (70\% - grupa ucząca, 30\% - grupa testowa). Sam model uczy się podczas 100 epok na zbiorze treningowym, gdzie każda epoka składa się z 55 iteracji z której każda ma rozmiar batch'a równy 100, dodatkowo przy każdej epoce model zostawia 20\% losowych rekordów na walidację.
...
Dane zostały losowo podzielone na zbiór treningowy (70\% obserwacji) oraz testowy (30\% obserwacji). Model został wytrenowany na zbiorze treningowym, natomiast jego skuteczność oceniono na niezależnym zbiorze testowym. Dodatkowo podczas procesu przycinania drzewa wykorzystano walidację krzyżową (k-folds dla k=10) w celu wyznaczenia optymalnego rozmiaru modelu i ograniczenia zjawiska przeuczenia.
...
W analizie wykorzystano algorytm KNN zaimplementowany w bibliotekach \texttt{caret} oraz \texttt{class}. Model działa przy użyciu standardowej dla naszej pracy pięciokrotnej walidacji Monte Carlo (70\% - grupa ucząca, 30\% - grupa testowa).
...
Ten model wykorzystuje pięciokrotną walidacje Monte Carlo, dzieląc wektory probabilistyczne otrzymane z ewaluacji submodeli na testowym zbiorze danych z naszych oryginalnych danych, w proporcjach 70:30 (70\% - grupa ucząca, 30\% - grupa testowa), dodatkowo sam model ma wbudowaną walidację Monte Carlo na samym zbiorze treningowym, zostawiając 10\% na walidacje w każdej ze 150 epok.
...
\subsection{Sieć neuronowa}
	...
	\caption{Macierz pomyłek dla metody sieci neuronowej.}
        \label{fig:both_images}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{../Plots/roc_curves/roc_Sieć MLP (R).png}
	\caption{Wykres krzywej ROC dla metody sieci neuronowej.}
        \label{fig:both_images}
    \end{figure}


Powyższe wyniki klasyfikatora MLP wskazują, że model osiąga umiarkowanie dobrą skuteczność w rozróżnianiu zwycięstwa drużyny niebieskiej i czerwonej. Dokładność na poziomie  oznacza, że prawie 3/4 wszystkich obserwacji została sklasyfikowana poprawnie. Jednocześnie około 27\% przypadków zostało zaklasyfikowanych błędnie, co wskazuje, że problem nie jest trywialny i model nie jest wolny od pomyłek. 

Czułość (Sensitivity = 71,27\%) pokazuje, że model poprawnie identyfikuje ponad 71\% zwycięstw niebieskiej drużyny. 

Swoistość (Specificity = 74,56\%) jest nieco wyższa od czułości, co wskazuje, że model lepiej rozpoznaje zwycięstwa
drużyny czerwonej niż niebieskiej. Różnica jest jednak niewielka, dlatego można stwierdzić, że model zachowuje względnie zrównoważoną skuteczność dla obu klas i nie wykazuje wyraźnej tendencji do faworyzowania jednej z nich. 

Precyzja (Precision = 73,36\%) świadczy to o stosunkowo dobrej wiarygodności pozytywnych predykcji. 

Wartość NPV (Negative Predictive Value = 72,53\%) wskazuje, że przewidywania zwycięstwa drużyny czerwonej również są poprawne w ponad 72\% przypadków. Oznacza to, że jakość predykcji obu klas jest zbliżona.

Krzywa ROC oraz średnia wartość AUC równa 0,805 świadczą o dobrej zdolności dyskryminacyjnej modelu. Model skutecznie odróżnia obserwacje należące do różnych klas i osiąga wynik wyraźnie lepszy od klasyfikatora losowego. Można więc stwierdzić, że klasyfikator jest dobrej jakości.

\subsection{Drzewo klasyfikacyjne}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.63\textwidth]{../Plots/confusion_matrixes/ClassificationTree.png}
...
%%%%%%%%%%%%
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.50\textwidth]{../Plots/roc_curves/roc_Drzewo klasyfikacyjne.png}
        \caption{Wykres krzywej ROC dla metody drzewa klasyfikacyjnego.}
        \label{fig:both_images}
    \end{figure}

Powyższe wyniki klasyfikatora drzewa klasyfikacyjnego wskazują, że model osiąga umiarkowanie dobrą skuteczność w rozróżnianiu zwycięstwa drużyny niebieskiej i czerwonej. Dokładność na poziomie  oznacza, że prawie 3/4 wszystkich obserwacji została sklasyfikowana poprawnie. Jednocześnie około 27\% przypadków zostało zaklasyfikowanych błędnie, co wskazuje, że problem nie jest trywialny i model nie jest wolny od pomyłek. 

Czułość (Sensitivity = 74,96\%) pokazuje, że model poprawnie identyfikuje poniżej 75\% zwycięstw niebieskiej drużyny. 

Swoistość (Specificity = 70,83\%) jest nieco niższa od czułości, co wskazuje, że model gorzej rozpoznaje zwycięstwa
drużyny czerwonej niż niebieskiej. Różnica jest jednak niewielka, dlatego można stwierdzić, że model zachowuje względnie zrównoważoną skuteczność dla obu klas i nie wykazuje wyraźnej tendencji do faworyzowania jednej z nich. 

Precyzja (Precision = 67,70\%) świadczy to o względnie słabej wiarygodności pozytywnych predykcji. 

Wartość NPV (Negative Predictive Value = 77,62\%) wskazuje, że przewidywania zwycięstwa drużyny czerwonej również są poprawne w ponad 77\% przypadków. Oznacza to, że jakość predykcji obu klas jest rozbieżna.

Krzywa ROC oraz średnia wartość AUC równa 0,782 świadczą o względnie dobrej zdolności dyskryminacyjnej modelu. Model skutecznie odróżnia obserwacje należące do różnych klas i osiąga wynik wyraźnie lepszy od klasyfikatora losowego. Można więc stwierdzić, że klasyfikator jest dobrej jakości. Ze względu na małą ilość gałęzi krzywa ROC jest widocznie podzielona na 4 odcinki co odpowiada ilości gałęzi drzewa.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.50\textwidth]{../Plots/tree_plots/tree_cv.png}
        \caption{Wykres błędu walidacji krzyżowej w zależności od rozmiaru drzewa.}
        \label{fig:both_images}
    \end{figure}

Na podstawie powyższych wykresów możemy stwierdzić, że najważniejszą cechą a zarazem jedyną szczególnie istotną jest zmienna \textit{diff\_PCA\_Conpontent\_1} czyli rezultat PCA  zmiennych opisujących różnice pomiędzy drużynami w złocie i doświadczeniu. Innymi słowy dla tego modelu najistotniejsze są złoto i doświadczenie.

Dodatkowo zwiekszenie złożoności modelu ponad dwie gałęzie nie przynosi poprawy rezultatów co można wywnioskować z wykresu błędu CV dla różnych rozmiarów drzewa oraz z klasyfikacji na nieprzyciętym drzewie.
...
\subsection{K najbliższych sąsiadów}
    ...
    \begin{figure}[H]
        ...
        \hfill
        \includegraphics[width=0.47\textwidth]{../Plots/roc_curves/roc_roc_knn.png}
	\caption{Wykres accuracy dla różnych wartości k oraz wykres krzywej ROC dla metody KNN.}
        \label{fig:both_images}
    \end{figure}

Powyższe wyniki klasyfikatora KNN wskazują, że model osiąga umiarkowanie dobrą skuteczność w rozróżnianiu zwycięstwa drużyny niebieskiej i czerwonej. Dokładność na poziomie  oznacza, że prawie 3/4 wszystkich obserwacji została sklasyfikowana poprawnie. Jednocześnie około 27\% przypadków zostało zaklasyfikowanych błędnie, co wskazuje, że problem nie jest trywialny i model nie jest wolny od pomyłek. 

Czułość (Sensitivity = 70,45\%) pokazuje, że model poprawnie identyfikuje ponad 70\% zwycięstw niebieskiej drużyny. 

Swoistość (Specificity = 72,62\%) jest nieco wyższa od czułości, co wskazuje, że model lepiej rozpoznaje zwycięstwa drużyny czerwonej niż niebieskiej. Różnica jest jednak niewielka, dlatego można stwierdzić, że model zachowuje względnie zrównoważoną skuteczność dla obu klas i nie wykazuje wyraźnej tendencji do faworyzowania jednej z nich. 

Precyzja (Precision = 71,59\%) świadczy to o stosunkowo dobrej wiarygodności pozytywnych predykcji. 

Wartość NPV (Negative Predictive Value = 71,50\%) wskazuje, że przewidywania zwycięstwa drużyny czerwonej również są poprawne w ponad 71\% przypadków. Oznacza to, że jakość predykcji obu klas jest zbliżona.

Z wykresu zależności celności od parametru k po walidacji modelu KNN możemy wywnioskować, że najlepsze k wynosi 83 i na takiej wartości będziemy opierali swój model.

Krzywa ROC oraz średnia wartość AUC równa 0,795 świadczą o dobrej zdolności dyskryminacyjnej modelu. Model skutecznie odróżnia obserwacje należące do różnych klas i osiąga wynik wyraźnie lepszy od klasyfikatora losowego. Można więc stwierdzić, że klasyfikator jest dobrej jakości.

\subsection{Metoda hybrydowa na bazie średniej arytmetycznej}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.65\textwidth]{../Plots/confusion_matrixes/Mixed_AVG_real_data.png}
	\caption{Macierz pomyłek dla metody mieszanej na bazie średniej arytmetycznej.}
        \label{fig:both_images}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.50\textwidth]{../Plots/roc_curves/roc_Model hybrydowy śr. (R).png}
	\caption{Wykres krzywej ROC dla metody mieszanej na bazie średniej arytmetycznej.}
        \label{fig:both_images}
    \end{figure}

Powyższe wyniki klasyfikatora hybrydowego na bazie średniej arytmetycznej wskazują, że model osiąga umiarkowanie dobrą skuteczność w rozróżnianiu zwycięstwa drużyny niebieskiej i czerwonej. Dokładność na poziomie  oznacza, że prawie 3/4 wszystkich obserwacji została sklasyfikowana poprawnie. Jednocześnie około 27\% przypadków zostało zaklasyfikowanych błędnie, co wskazuje, że problem nie jest trywialny i model nie jest wolny od pomyłek. 

Czułość (Sensitivity = 71,91\%) pokazuje, że model poprawnie identyfikuje blisko 72\% zwycięstw niebieskiej drużyny. 

Swoistość (Specificity = 73,04\%) jest nieco wyższa od czułości, co wskazuje, że model lepiej rozpoznaje zwycięstwa drużyny czerwonej niż niebieskiej. Różnica jest jednak niewielka, dlatego można stwierdzić, że model zachowuje względnie zrównoważoną skuteczność dla obu klas i nie wykazuje wyraźnej tendencji do faworyzowania jednej z nich. 

Precyzja (Precision = 72,42\%) świadczy to o stosunkowo dobrej wiarygodności pozytywnych predykcji. 

Wartość NPV (Negative Predictive Value = 72,54\%) wskazuje, że przewidywania zwycięstwa drużyny czerwonej również są poprawne w ponad 72\% przypadków. Oznacza to, że jakość predykcji obu klas jest zbliżona.

Krzywa ROC oraz średnia wartość AUC równa 0,804 świadczą o dobrej zdolności dyskryminacyjnej modelu. Model skutecznie odróżnia obserwacje należące do różnych klas i osiąga wynik wyraźnie lepszy od klasyfikatora losowego. Można więc stwierdzić, że klasyfikator jest dobrej jakości.

\subsection{Metoda hybrydowa na bazie sieci neuronowej}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.60\textwidth]{../Plots/confusion_matrixes/Mixed_NN_real_data.png}
	\caption{Macierz pomyłek dla metody mieszanej na bazie sieci neuronowej.}
        \label{fig:both_images}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.50\textwidth]{../Plots/roc_curves/roc_Model hybrydowy MLP(R).png}
	\caption{Wykres krzywej ROC dla metody mieszanej na bazie sieci neuronowej.}
        \label{fig:both_images}
    \end{figure}

Powyższe wyniki klasyfikatora hybrydowego MLP wskazują, że model osiąga umiarkowanie dobrą skuteczność w rozróżnianiu zwycięstwa drużyny niebieskiej i czerwonej. Dokładność na poziomie  oznacza, że prawie 3/4 wszystkich obserwacji została sklasyfikowana poprawnie. Jednocześnie około 27\% przypadków zostało zaklasyfikowanych błędnie, co wskazuje, że problem nie jest trywialny i model nie jest wolny od pomyłek. 

Czułość (Sensitivity = 70,09\%) pokazuje, że model poprawnie identyfikuje ponad 70\% zwycięstw niebieskiej drużyny. 

Swoistość (Specificity = 74,36\%) jest nieco wyższa od czułości, co wskazuje, że model lepiej rozpoznaje zwycięstwa
drużyny czerwonej niż niebieskiej. Różnica jest jednak niewielka, dlatego można stwierdzić, że model zachowuje względnie zrównoważoną skuteczność dla obu klas i nie wykazuje wyraźnej tendencji do faworyzowania jednej z nich. 

Precyzja (Precision = 73,44\%) świadczy to o stosunkowo dobrej wiarygodności pozytywnych predykcji. 

Wartość NPV (Negative Predictive Value = 71,08\%) wskazuje, że przewidywania zwycięstwa drużyny czerwonej również są poprawne w ponad 71\% przypadków. Oznacza to, że jakość predykcji obu klas jest zbliżona.

Krzywa ROC oraz średnia wartość AUC równa 0,802 świadczą o dobrej zdolności dyskryminacyjnej modelu. Model skutecznie odróżnia obserwacje należące do różnych klas i osiąga wynik wyraźnie lepszy od klasyfikatora losowego. Można więc stwierdzić, że klasyfikator jest dobrej jakości.

\subsection{Porównanie skuteczności metod}

Powyżej przedstawione rezultaty zostały zebrane do poniższej tabeli. Uwzględniamy jedynie celność i miarę AUC.
...
\begin{tabular}{l c c}
	...
	\midrule
	Sieć neuronowa & \getAccuracyNn\% & \getAucNn \\ 
	Drzewo klasyfikacyjne & \getAccuracyTree\% & \getAucTree \\
	KNN  & \getAccuracyKnn\% & \getAucKnn \\
	hybr. sieć neuronowa  & \getAccuracyMixednn\% & \getAucMixedNn\\
...
\begin{tabular}{l *{5}{c}}
    ...
    \midrule
	\texttt{Drzewo klasyfikacyjne} & 1 & 0 & 0 & 1 & 1 \\
	\texttt{Sieć neuronowa} & 1 & 0 & 0 & 1 & 1 \\
	\texttt{hybr. średnia} & 1 & 0 & 0 & 1 & 1 \\
	\texttt{KNN} & 1 & 0 & 0 & 1 & 1 \\
	\texttt{blueWins} & 1 & 0 & 0 & 1 & 1 \\
	blueWardsPlaced & 52 & 16 & 13 & 17 & 13 \\
	blueWardsDestroyed & 3 & 5 & 2 & 3 & 4 \\
	blueDragons & 0 & 0 & 0 & 1 & 0 \\
	blueHeralds & 1 & 0 & 0 & 0 & 0 \\
	blueTotalJungleMinionsKilled & 33 & 40 & 36 & 46 & 68 \\
	blueCSPerMin & 21.8 & 23.0 & 19.9 & 19.9 & 27.0 \\
	redWardsPlaced & 21 & 19 & 35 & 18 & 94 \\
	redWardsDestroyed & 2 & 3 & 1 & 3 & 2 \\
	redFirstBlood & 0 & 0 & 1 & 0 & 1 \\
	redDragons & 1 & 1 & 1 & 0 & 1 \\
	redHeralds & 0 & 1 & 0 & 0 & 1 \\
	redTotalJungleMinionsKilled & 52 & 60 & 75 & 40 & 44 \\
	redCSPerMin & 22.0 & 24.2 & 19.6 & 20.9 & 20.9 \\
	blueKA & 12 & 4 & 9 & 20 & 9 \\
	redKA & 0 & 18 & 13 & 14 & 2 \\
	diff\_PCA\_Component\_1 & -1.37 & 3.36 & 0.65 & -0.77 & -1.35 \\
    \bottomrule
\end{tabular}
\caption{Pięć syntetycznych rekordów i ich wyniki predykcji.}
\end{table}

Na podstawie wyników przedstawionych w tabeli można stwierdzić, że wszystkie zastosowane modele wygenerowały identyczne predykcje dla każdego z pięciu syntetycznych rekordów. Otrzymane klasy były zgodne z wartością zmiennej docelowej \texttt{blueWins}, co wskazuje na wysoką spójność oraz poprawność działania modeli na danych syntetycznych. Wyniki sugerują, że opracowane modele skutecznie odwzorowują zależności występujące w danych i są zdolne do prawidłowej klasyfikacji nowych obserwacji wygenerowanych metodą syntezy danych.
...
Stanowi to dobry rezultat, potwierdzający, że dane z wczesnej fazy meczu zawierają istotną informację predykcyjną, jednak do powiększenia skuteczności i poprawności predykcji niezbędnę są informacje późniejszym przebiegu rozgrywki. Potwierdza to istotność późniejszego etapu gry i, że pierwsze 10 minut jest ważne, ale nie determinuje jednoznacznie wyniku.

Również w odniesieniu do innych prac na tym samym zbiorze danych, jak \textit{LoL: How to Win} \cite{wang_lol_win}, osiągnęliśmy porównywalnie dobry wynik, co autorzy korzystający z innych metod klasyfikacji. Dodatkowo nasza najskuteczniejsza metoda, sieć neuronowa, przewyższa wyniki uzyskane w analizie skupień na tym samym zbiorze danych, gdzie celność k-means wyniosła 70,88\%, a model mieszanin Gaussa estymowany algorytmem EM --- 72,46\% \cite{projekt1}. Potwierdza to, że zastosowanie dedykowanych klasyfikatorów pozwala na skuteczniejsze przewidywanie wyniku meczu niż metody nienadzorowane.
...
Dodatkowo dla modelu drzewa decyzyjnego najistotniejszą zmienną było \textit{diff\_PCA\_Component\_1}, co wskazuje, że, abstachując od subtelniejszych powiązań danych cech ze zwycięstwem, jest jedyną skuteczną. 

Nasza analiza klasyfikacyjna potwierdza ogólnie przyjęte założenie, że do wygranej potrzeba dużo zabójstw, mało śmierci i dużo asyst, co dodatkowo uzupełnia analizy dokonane w innych publikacjach co do głównych determinant wygranej bądź porażki na podstawie danych z początku rozgrywki \cite{gasmi_lol_first10, wang_lol_win, arancio_decisions_10min}. Dodatkowo pokazuje, że metody klasyfikacji zapewniają zadowalające odzwierciedlenie faktycznego rezultatu meczu. 
...
% pierwszy projekt 
\bibitem{projekt1}
Michał Wojda, Sebastian Śmietański, Tomasz Teofilak, Tomasz Żołądek,
\textit{Analiza skupień meczów League of Legends przy ograniczonych danych z pierwszych 10 minut meczu},
GitHub 2026.  
Dostęp: \url{https://github.com/echooff2/MADProj1/blob/main/latex/projekt1.pdf}
