...
\begin{itemize}
\item wybór zmiennych wykorzystanych w procesie klasyfikacji,
\item wstępną analizę danych,
\item zastosowanie metod klasyfikacji: [NAZWA METODY 1], [NAZWA METODY 2], [NAZWA METODY 3] oraz [NAZWA METODY MIESZANEJ],
\item analizę i interpretację wyników klasyfikacji,
\item ocenę skuteczności modeli przy użyciu wybranych miar jakości klasyfikacji [NAZWY METOD],
\item porównanie skuteczności zastosowanych metod.
\end{itemize}
...
Obie prace uzasadniają zastosowane w niniejszej pracy podejście, wskazując, że dane
z pierwszych minut zawierają informację predykcyjną, a ograniczony zestaw cech ilościo-
wych może ujawniać strukturę meczów. W konsekwencji zastosowanie metod klasteryzacji
(k-means oraz modele mieszanin Gaussa estymowane algorytmem EM) jest uzasadnione
jako narzędzie eksploracji danych i oceny, czy wczesne statystyki pozwalają odróżnić me-
cze wygrane i przegrane.
...
Zbiór obejmuje mecze rankingowe rozgrywane na wysokim poziomie rankingowym — od rangi Diamond wzwyż, co odpowiada około 5\% najlepszych graczy. Dane pochodzą z okresu sprzed około sześciu lat. Autor zbioru nie określa dokładnego regionu serwerowego, z którego zostały pobrane mecze.
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=1.0\textwidth]{../Repo/Plots/corrmatrixes/WykresKorelacji1.png}
    }
...
\begin{figure}[H]
    ...
    \caption{Macierz korelacji dla wszystkich zmiennych 2.}
\end{figure}
...
W celu ujednolicenia zakresów wartości poszczególnych cech zastosowano metodę skalowania Min-Max. Proces ten polega na przekształceniu wartości zmiennych do ustalonego przedziału, najczęściej od 0 do 1. Skalowanie danych jest szczególnie istotne w przypadku metod uczenia maszynowego wrażliwych na zakres wartości cech.

Transformacja Min-Max została wykonana zgodnie ze wzorem:
...
\begin{itemize}
    ...
    \item $x_{\min}$ --- najmniejsza wartość danej cechy,
    \item $x_{\max}$ --- największa wartość danej cechy,
    \item $x'$ --- wartość po przeskalowaniu.
\end{itemize}

W wyniku zastosowania skalowania wszystkie analizowane cechy numeryczne zostały sprowadzone do wspólnego zakresu wartości. Pozwoliło to ograniczyć wpływ różnic skali pomiędzy zmiennymi na działanie modeli klasyfikacyjnych.

\subsection{Usunięcie zmiennych}
